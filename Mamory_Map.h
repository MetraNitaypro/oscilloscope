...
typedef enum
{
  MM_VAR_H_MOST_ACTIVE_CHNS = 0,
  MM_VAR_BOOST_ACTIVE_CHNS,
  MM_VAR_BOOST_MODE,
  MM_VAR_RAW_DATA_TRAN_SIZE,
  MM_VARS_COUNT
}
MM_varId_e;
...
typedef union{
  struct{ //Биты статуса
    ...
    bool sensErrREF15 : 1; //Ошибка датчика REF15
    bool sensErrT1 : 1; //Ошибка датчика T1
    bool sensErrT2 : 1; //Ошибка датчика T2
    bool sensErrT3 : 1; //Ошибка датчика T3
    bool sensErrT4 : 1; //Ошибка датчика T4
    bool panelErr : 1; //Ошибка панели управления
    bool chargeCurrLim : 1; //Превышен ток выхода на режим
...
void MM_writeVal( const MM_varId_e varId, const void * src, const uint16_t size );
void MM_writeValWithOffset( const MM_varId_e varId, const void * src, const uint16_t offset, const uint16_t size );
void MM_readVal( const MM_varId_e varId, void * const dst, const uint16_t size );
void MM_readValWithOffset( const MM_varId_e varId, void * const dst, const uint16_t offset, const uint16_t size );
